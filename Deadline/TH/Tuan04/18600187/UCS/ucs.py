    ...
    def UCS(self, start, end):
        ...
        for i in range(0, len(self.Matrix[0])):
            if(self.Matrix[start][i] != 0):
                visited[i] = True
                pq.append([self.Matrix[start][i], str(start) + str(i)])
        ...
        while len(pq) > 0 and int(pq[0][1][-1]) != end:
            ...
            for i in range(0, len(self.Matrix[0])):
                if(self.Matrix[int(temp[1][-1])][i] != 0):
                    visited[i] = True
                    t = temp.copy()
                    t[0] = t[0] + self.Matrix[int(temp[1][-1])][i]
                    t[1] = t[1] + str(i)
                    pq.append(t)
            pq.sort()
        if len(pq) == 0:
...
def main():
    ...
    with open("ucs_output.txt", "w") as f:
        arr = g.UCS(g.SE[0], g.SE[1])
        if(arr == -1):
            f.write("Weight: ")
            f.write("\n")
            f.write("Path: ")
        else:
            f.write("Weight: " + str(arr[0]))
            f.write("\n")
            f.write("Path: ")
